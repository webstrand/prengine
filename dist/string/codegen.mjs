...
export function codegen(str, namespace = "_") {
    /**
     ...
     */
    let concat = "";
    if (!parse(str, visitString, visitExpr))
        return null;
    function visitString(string, offset) {
        concat += "+" + JSON.stringify(string);
    }
    function visitExpr(untrimmedExpr, offset) {
        ...
        if (expr.length === 0 || expr.includes("return") || expr[0] === ";" || expr[expr.length - 1] === ";") {
            ...
            if (expr.includes("_ebiwl315yn9p4145wymd31rvej"))
                throw new Error("Expression must not contain escape token");
...
/** @internal */
export function countClosingParens(str) {
    let count = 0;
    let lastCloseParen = -1;
    while ((lastCloseParen = str.indexOf(")", lastCloseParen + 1)) !== -1)
        count++;
    return count;
}
/** @internal */
export function countClosingBraces(str) {
    let count = 0;
    let lastCloseBrace = -1;
    while ((lastCloseBrace = str.indexOf("}", lastCloseBrace + 1)) !== -1)
        count++;
    return count;
}
